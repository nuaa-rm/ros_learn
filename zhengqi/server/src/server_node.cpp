...
{
    //打印接收到的数据
    ROS_INFO("Subsrcibe data name:%s,time:%d",msg->name.c_str(),msg->time_now.sec);
}
                                         ...
                                         client::Crist::Response &res)
{
   ...
   //显示请求数据
    ...
    ros::Subscriber inform_msg_sub=node.subscribe<client::topic_tim>("/info_msg",10,show_data);
}
int main(int argc,char **argv)
{
    ...
    ros::ServiceServer  name_server=node.advertiseService("/show_name",Callback);
    //等待循环回调函数
    ros::spin();
