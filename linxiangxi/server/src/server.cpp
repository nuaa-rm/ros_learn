...
void client_message_callback(const client::client_message & client_message){
    ...
    strftime(output_tmp, sizeof(output_tmp), "%Y-%m-%d %H:%M:%S", localtime(&_time));
    ROS_INFO("name: %s, time: %s", client_message.name.c_str(), output_tmp);
}

bool client_login(client::client_login::Request & req, client::client_login::Response & res){
    if(req.require_code == LOGIN){
        ROS_INFO("login request from %s", req.name.c_str());
        Client_info info;
        info.client_sub = (*server_node_handle_ptr).subscribe(req.name, SUBSCRIBER_SIZE, client_message_callback);
...
int main(int argc, char ** argv){
    ...
    ros::ServiceServer server_login = server_node_handle.advertiseService("client_login", client_login);
