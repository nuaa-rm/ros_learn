                    ...
                    server::Register::Response &res)
{
    ...
    if (sub_map.find(client_name) != sub_map.end()) {
        ROS_INFO("Register Error,Client %s already exists!", client_name.c_str());
        res.success = false;
    } else {
        ros::NodeHandle n;
        std::string topic_name = client_name;

        ros::Subscriber sub = n.subscribe<std_msgs::String>(topic_name, 1000, chatterCallback);
...
int main(int argc, char** argv){
    ...
    ros::ServiceServer register_service = n.advertiseService("register", registerClient);
    ros::ServiceServer unregister_service = n.advertiseService("unregister", unregisterClient);
